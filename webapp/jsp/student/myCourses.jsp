...
<div class="container-fluid mt-1">
    <a class="lead">User select course for : <kbd> ${selected}</kbd> </a>
    <table class="table table-striped table-hover">
        <tr>
            ...
            <th class="col-sm-1 p-3 text-center"> delete course</th>
        </tr>
        <c:forEach var="cs" items="${courseStudent}" varStatus="counter">
            <tr>
                <td class="col-sm-1 p-3 text-center"> ${counter.count}</td>
                <td class="col-sm-1 p-3 text-center">
                    <c:out value="${cs.teacherCourse.teacherSubject.subject.nameSubject}"/>
                </td>
                <td class="col-sm-1 p-3 text-center">
                    <c:out value="${cs.teacherCourse.teacherSubject.userInfo}"/>
                </td>
                <td class="col-sm-1 p-3 text-center">
                    <c:out value="${cs.userInfo}"/>
                </td>
                <td class="col-sm-1 p-3 text-center">
                    <c:out value="${cs.teacherCourse.startDate}"/>
                </td>
                <td class="col-sm-1 p-3 text-center">
                    <c:out value="${cs.teacherCourse.endDate}"/>
                </td>
                <td class="col-sm-1 p-3 text-center">
                    <c:out value="${cs.status}"/>
                </td>
                <td class="col-sm-1 p-3 text-center">
                <form method="POST" action="/webProject/my">
                    <button class="btn btn-primary btn-secondary" type="submit" name="command">
                        <input type="hidden" name="find" value="${cs.id}"/>
                        DELETE
                    </button>
                </form>
                </td>
