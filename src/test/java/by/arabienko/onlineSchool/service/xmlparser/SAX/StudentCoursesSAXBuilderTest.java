package by.arabienko.onlineSchool.service.xmlparser.SAX;
...
public class StudentCoursesSAXBuilderTest {

    StudentCoursesSAXBuilder coursesSAXBuilder = new StudentCoursesSAXBuilder();

    @DataProvider(name = "parsing_SAX")
    ...
    @Test(description = "positive scenario",
            dataProvider = "parsing_SAX")
    public void testBuildStudentCurses(String file,
                                       Set<StudentCourse> actual) {
        coursesSAXBuilder.buildStudentCurses(file);
        Set<StudentCourse> expected =
                coursesSAXBuilder.getStudentCourses();
