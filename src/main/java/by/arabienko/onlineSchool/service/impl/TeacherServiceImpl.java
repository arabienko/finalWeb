...
public class TeacherServiceImpl implements UserService {
    ...
    public Optional<User> checkUserByLoginPassword(
            String login, String password) throws ExceptionService, PersistentException {
        ...
        try {
            ...
            transaction.createDao(teacher);
            result = teacher.findEntityByParam(login, DigestUtils.sha512Hex(password));
            transaction.commit();
    ...
    public boolean isLoginUnique(String patternLogin)
        ...
        try {
            transaction = factory.createTransaction();
            transaction.createDao(teacher);
            result = teacher.isLoginUnique(patternLogin);
            transaction.commit();
        } catch (DaoException | PersistentException e) {
    ...
    public boolean registerUser(User user)
        ...
        try {
            transaction = factory.createTransaction();
            transaction.createDao(teacher);
            result = teacher.registerUser(user);
            transaction.commit();
        } catch (DaoException | PersistentException e) {
    ...
    public User findById(Long id) throws ExceptionService, PersistentException {
        ...
        try {
            transaction = factory.createTransaction();
            transaction.createDao(teacher);
            result = teacher.findEntityById(id);
            transaction.commit();
        } catch (DaoException | PersistentException e) {
    ...
    public User findByEntityId(long id) throws ExceptionService, PersistentException {
        ...
        try {
            transaction = factory.createTransaction();
            transaction.createDao(teacher);
            result = teacher.findTeacherById(id);
            transaction.commit();
        } catch (DaoException | PersistentException e) {
